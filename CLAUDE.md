...
- **Confirmed working as of 2026-03-29 ~07:00 MEST** (~05:00 UTC). Runs GREEN. Wallet coverage: 121 (JSON) / 136 (DB), 32 smart money entities. Commit 52f2fd1 (UI polish). Whale txs: 11,597. ~274 total alerts (ID ~303). 47 raw evaluated / 14 deduped unique signals. exit_hype deduped: **50%/89%/78%** (10 signals). dip_buy: **100%/—/—** (1 deduped / 2 trades — FET both correct). buying_fear: **33%/0%/33%** (3 deduped). **Paper P&L: +$121.37** (2 FET dip_buy trades, both hit +5% profit target). ONDO 73-75% exit_hype WRONG at 24h (+3.5-4%), 48h eval due Mar 29 14-19 UTC. FET +8.8% ($0.2457). IMX buying_fear 0/3 all timeframes — ignore. Market mood: 9 (Extreme Fear). BTC: $66,684 (+0.72%).
...
- **buying_fear requires 2+ independent entities**: Single-entity buying was 0/5 = 0% hit rate (FET $9.2M Binance treasury ops created false 95% confidence). Now requires at least 2 independent entities with positive net buying. Single-entity dominance (>80% of total buying) gets 50% confidence discount. Entity diversity scales confidence: 2 entities = 67%, 3+ = 100%. **Never trust single-entity whale moves as accumulation signals.**
...
- **Current hit rates (as of 2026-03-27 15:26 MEST, 35 raw signals evaluated + 4 pending)**: exit_hype: **48%/67%/71%** at 24h/48h/72h (23 signals). excl LINK: **58%/76%/85%**. excl LINK+COMP: **61%/81%/92%**. **Bounce-back pattern confirmed 5/5**: IDs 283-287 all wrong/noise at 24h but correct at 48h. LINK 283 confirmed through 72h (-5.86%). buying_fear: **8%/27%/18%** (12 signals — IMX 288 wrong at 24h). dip_buy: 0 signals. Signal coins: SHIB, 1INCH, ARB, LDO, SAND, COMP, LINK, PEPE, AAVE, + IMX, FET (buying_fear). Dashboard deduplicates by coin+type keeping most recent (ts DESC).
...
**Section order**: Market Mood → Intelligence Alerts → Signal Track Record → Paper Trading → Top Movers → Social Buzz + Whale Activity (side-by-side) → Trending → Narratives
...
- **Whale tracker N+1 snapshot queries caused timeout**: `_estimate_usd()` called `db.get_latest_snapshot()` per-transaction — stETH looked up 12x, ETH 8x per run. Latest run hit 14-min timeout. Fixed: batch-fetch all token prices once at start of `collect()` using `db.get_latest_prices()`. **Never query individual snapshots in a loop — always batch.**
...
- **buying_fear pattern is 0/10 at 24h, 0/6 at 48h**: Fundamentally broken — whale buying during fear = catching falling knives, not detecting bottoms. FET dropped -12% at 48h despite $9.2M whale buying. UNI dropped -8%. Multi-entity gate exists in local code (not yet deployed) but even that may not fix the core issue. **Consider disabling buying_fear entirely or requiring 3+ entities + sustained buying across multiple analysis runs.**
- **Uncommitted code on local machine**: Multi-entity buying_fear gate (`smart_money.py`) and batch price lookup (`whale_tracker.py`) are modified locally but NOT pushed to GitHub Actions. GitHub Actions still runs the OLD code without these fixes.
- **Whale Alert API key**: not yet configured (needs paid plan from https://whale-alert.io)
- **Intelligence quality improves with data volume**: system has been running continuously since 2026-03-13 ~16:00 UTC (~6 days). Social baselines are reliable for major ERC-20 tokens. exit_hype generating validated signals across 4+ coins (SHIB, LDO, SAND, ARB). Full confidence in exit_hype signals; buying_fear needs fundamental redesign. **1 week of clean data target: ~2026-03-23.**
